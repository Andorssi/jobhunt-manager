...
export default function Home() {
  ...
  const sortedApplications = useMemo(() => {
    return [...applications].sort((a, b) => getNearestDeadline(a) - getNearestDeadline(b));
  }, [applications]);
  ...
  async function updateApplication(id: string, key: keyof Omit<Application, "id">, value: string) {
    setApplications(applications.map((app) => (app.id === id ? { ...app, [key]: value } : app)));
    ...
    const { error } = await supabase
      .from("applications")
      .update({
        [columnMap[key]]: key === "esDeadline" || key === "testDeadline" || value === "" ? value || null : value,
      })
      .eq("id", id);
    if (error) {
      console.error(error);
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-gray-50 p-6 text-gray-900">
        <section className="w-full max-w-md rounded-xl bg-white p-8 shadow">
          ...
          <p className="mb-6 text-sm text-gray-600">Googleアカウントでログインしてください。</p>
          <button onClick={signInWithGoogle} className="w-full rounded bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700">
            Googleでログイン
          </button>
        </section>
      </main>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">ログイン中: {user.email}</p>
          </div>
          <button onClick={signOut} className="rounded bg-gray-700 px-4 py-2 text-white hover:bg-gray-800">
            ログアウト
          </button>
        </div>

        <section className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          <div className="rounded-xl bg-white p-4 shadow"><p className="text-sm text-gray-500">期限切れ</p><p className="text-3xl font-bold text-red-600">{summary.overdue}</p></div>
          <div className="rounded-xl bg-white p-4 shadow"><p className="text-sm text-gray-500">3日以内</p><p className="text-3xl font-bold text-yellow-600">{summary.within3Days}</p></div>
          <div className="rounded-xl bg-white p-4 shadow"><p className="text-sm text-gray-500">ES未完了</p><p className="text-3xl font-bold">{summary.incompleteEs}</p></div>
          <div className="rounded-xl bg-white p-4 shadow"><p className="text-sm text-gray-500">検査未完了</p><p className="text-3xl font-bold">{summary.incompleteTest}</p></div>
        </section>

        <section className="mb-8 rounded-xl bg-white p-6 shadow">
          <h2 className="mb-4 text-xl font-semibold">企業情報を追加</h2>
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div><label className="mb-1 block text-sm font-medium">企業名 <span className="text-red-500">*</span></label><input className="w-full rounded border p-2" placeholder="例: NTT東日本" value={form.companyName} onChange={(e) => setForm({ ...form, companyName: e.target.value })} /></div>
            <div><label className="mb-1 block text-sm font-medium">マイページID</label><input className="w-full rounded border p-2" placeholder="例: AB123456" value={form.mypageId} onChange={(e) => setForm({ ...form, mypageId: e.target.value })} /></div>
            <div><label className="mb-1 block text-sm font-medium">ES締切日</label><input className="w-full rounded border p-2" type="date" value={form.esDeadline} onChange={(e) => setForm({ ...form, esDeadline: e.target.value })} /></div>
            <div><label className="mb-1 block text-sm font-medium">ESステータス</label><select className="w-full rounded border p-2" value={form.esStatus} onChange={(e) => setForm({ ...form, esStatus: e.target.value })}>{esStatuses.map((status) => <option key={status}>{status}</option>)}</select></div>
            <div><label className="mb-1 block text-sm font-medium">適性検査種別</label><select className="w-full rounded border p-2" value={form.testType} onChange={(e) => setForm({ ...form, testType: e.target.value })}>{testTypes.map((type) => <option key={type}>{type}</option>)}</select></div>
            <div><label className="mb-1 block text-sm font-medium">適性検査締切日</label><input className="w-full rounded border p-2" type="date" value={form.testDeadline} onChange={(e) => setForm({ ...form, testDeadline: e.target.value })} /></div>
            <div><label className="mb-1 block text-sm font-medium">検査ステータス</label><select className="w-full rounded border p-2" value={form.testStatus} onChange={(e) => setForm({ ...form, testStatus: e.target.value })}>{testStatuses.map((status) => <option key={status}>{status}</option>)}</select></div>
            <div><label className="mb-1 block text-sm font-medium">マイページURL</label><input className="w-full rounded border p-2" placeholder="https://..." value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} /></div>
            <div className="md:col-span-2"><label className="mb-1 block text-sm font-medium">メモ</label><textarea className="w-full rounded border p-2" placeholder="例: 適性検査AはTAL、適性検査Bは玉手箱" value={form.memo} onChange={(e) => setForm({ ...form, memo: e.target.value })} /></div>
          </div>
