  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 text-gray-900">
      <div className="mx-auto max-w-7xl">
        <h1 className="mb-6 text-3xl font-bold">就活ID管理表</h1>
        ...
        <section className="mb-8 rounded-xl bg-white p-6 shadow">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              ...
              <select className="w-full rounded border p-2" value={form.esStatus} onChange={(e) => setForm({ ...form, esStatus: e.target.value })}>
                {esStatuses.map((status) => <option key={status}>{status}</option>)}
              </select>
            </div>

            <div>
              <label className="mb-1 block text-sm font-medium">適性検査種別</label>
              <select className="w-full rounded border p-2" value={form.testType} onChange={(e) => setForm({ ...form, testType: e.target.value })}>
                {testTypes.map((type) => <option key={type}>{type}</option>)}
              </select>
            </div>
            ...
            <div>
              ...
              <select className="w-full rounded border p-2" value={form.testStatus} onChange={(e) => setForm({ ...form, testStatus: e.target.value })}>
                {testStatuses.map((status) => <option key={status}>{status}</option>)}
              </select>
            </div>
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-gray-100">
                  <th className="border p-2">企業名</th>
                  <th className="border p-2">ID</th>
                  <th className="border p-2">ES締切</th>
                  <th className="border p-2">ESステータス</th>
                  <th className="border p-2">適性検査種別</th>
                  <th className="border p-2">検査締切</th>
                  <th className="border p-2">検査ステータス</th>
              ...
              <tbody>
                  ...
                  return (
                    <tr key={app.id}>
                      <td className="border p-2 font-medium">
                        {isEditing ? <input className="w-36 rounded border p-1" value={app.companyName} onChange={(e) => updateApplication(app.id, "companyName", e.target.value)} /> : app.companyName}
                      </td>
                      <td className="border p-2">
                        {isEditing ? <input className="w-32 rounded border p-1" value={app.mypageId} onChange={(e) => updateApplication(app.id, "mypageId", e.target.value)} /> : app.mypageId}
                      </td>
                      <td className={`border p-2 ${getDeadlineClass(app.esDeadline, app.esStatus)}`}>
                        {isEditing ? <input className="w-36 rounded border p-1" type="date" value={app.esDeadline} onChange={(e) => updateApplication(app.id, "esDeadline", e.target.value)} /> : app.esDeadline}
                      </td>
                      <td className="border p-2">
                        {isEditing ? <select className="w-28 rounded border p-1" value={app.esStatus} onChange={(e) => updateApplication(app.id, "esStatus", e.target.value)}>{esStatuses.map((status) => <option key={status}>{status}</option>)}</select> : app.esStatus}
                      </td>
                      <td className="border p-2">
                        {isEditing ? <select className="w-28 rounded border p-1" value={app.testType} onChange={(e) => updateApplication(app.id, "testType", e.target.value)}>{testTypes.map((type) => <option key={type}>{type}</option>)}</select> : app.testType}
                      </td>
                      <td className={`border p-2 ${getDeadlineClass(app.testDeadline, app.testStatus)}`}>
                        {isEditing ? <input className="w-36 rounded border p-1" type="date" value={app.testDeadline} onChange={(e) => updateApplication(app.id, "testDeadline", e.target.value)} /> : app.testDeadline}
                      </td>
                      <td className="border p-2">
                        {isEditing ? <select className="w-28 rounded border p-1" value={app.testStatus} onChange={(e) => updateApplication(app.id, "testStatus", e.target.value)}>{testStatuses.map((status) => <option key={status}>{status}</option>)}</select> : app.testStatus}
                      </td>
                      <td className="border p-2">
                        {isEditing ? <input className="w-48 rounded border p-1" value={app.url} onChange={(e) => updateApplication(app.id, "url", e.target.value)} /> : app.url ? <a className="text-blue-600 underline" href={app.url} target="_blank" rel="noreferrer">開く</a> : ""}
                      </td>
                      <td className="border p-2">
                        {isEditing ? <textarea className="w-48 rounded border p-1" value={app.memo} onChange={(e) => updateApplication(app.id, "memo", e.target.value)} /> : app.memo}
                      </td>
                      <td className="border p-2">
                        <div className="flex gap-2">
                          {isEditing ? (
                            <button onClick={() => setEditingId(null)} className="rounded bg-green-600 px-3 py-1 text-white hover:bg-green-700">完了</button>
                          ) : (
                            <button onClick={() => setEditingId(app.id)} className="rounded bg-blue-600 px-3 py-1 text-white hover:bg-blue-700">編集</button>
                          )}
                          <button onClick={() => {
                            if (window.confirm(`${app.companyName} を削除しますか？`)) {
                              deleteApplication(app.id);
                            }
                          }} className="rounded bg-red-500 px-3 py-1 text-white hover:bg-red-600">削除</button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                {applications.length === 0 && (
                  <tr>
                    <td className="border p-4 text-center" colSpan={10}>まだ登録がありません。</td>
                  </tr>
                )}
              </tbody>
            </table>
